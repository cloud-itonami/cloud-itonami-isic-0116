(ns fibreops.governor
  "Field Operations Governor -- the independent compliance layer that
  earns the FibreOpsAdvisor the right to commit. The LLM has no notion
  of:
    ...
    - Whether a logged field-record acreage is a plausible positive
      observation
    - Whether a logged field-record quality-grade is a recognized grade
      code
  ...
  CRITICAL: Any proposal to flag a crop-health concern (pest such as boll
  weevil, disease, drought-stress) ALWAYS escalates to a human
  ...
  Hard violations (always HOLD, no override, permanent):
    ...
    6. `:log-field-record` with an unrecognized quality-grade code
  ...
  specializes fibre-crop-farm back-office coordination concerns (field
  registration, closed op allowlist, equipment/pesticide-decision
  exclusion, cost threshold, fibre-quality-grade vocabulary) rather than
  cereal-growing concerns."
...
(defn- quality-grade-invalid-violations
  "For `:log-field-record`, INDEPENDENTLY verify a logged fibre-quality
  grade is one of the actor's recognized closed vocabulary
  (`fibreops.facts/fibre-quality-grades`) via
  `registry/quality-grade-unknown?`. Evaluated only when a
  ...
  (when (and (= :log-field-record (:op proposal))
    ...
    [{:rule :quality-grade-invalid
      :detail (str "品質等級（quality-grade） " (pr-str (:quality-grade proposal)) " は認識済みの等級コードではない -- 記録提案は進められない")}]))

(defn- cost-threshold-for
  "Resolve the escalation threshold for a supply-order proposal: the
  category-specific threshold from `fibreops.facts` if the category is
...
(defn check
  "Censors a FibreOpsAdvisor proposal against the Governor rules. Returns
