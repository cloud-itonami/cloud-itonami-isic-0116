(ns fibreops.operation
  ...
  actor run. The advisor (FibreOpsAdvisor) is sealed into a single node
  ...
  One run = one fibre-crop-growing coordination operation (intake ->
  advise -> govern -> decide -> commit | hold | approval). No unbounded
  inner loop -- each operation is auditable and checkpointed. A field's
  operating history is advanced by MANY operations (log-field-record /
  ...
  is clean -- see `fibreops.governor/always-escalate-ops`.
  ...
  (:require [fibreops.advisor :as advisor]
...
(defn run-operation
  "Run one fibre-crop-growing operation through the advisor -> governor ->
