(ns fibreops.phase
  "Phase gate for the fibre-crop-growing operations coordinator rollout
  (0->3 maturity). Mirrors `cerealops.phase` (cloud-itonami-isic-0111) in
  ...
  Phase 1: supervised operation -- always-escalate ops
    (`fibreops.governor/always-escalate-ops`) escalate even when clean;
  ...
  (:require [fibreops.governor :as governor]))
