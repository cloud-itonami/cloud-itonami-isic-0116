(ns fibreops.advisor
  "FibreOpsAdvisor -- the contained LLM/decision node. This actor's
  intelligence layer proposes back-office coordination actions (field
  record logging, planting/defoliation/retting/harvest scheduling, crop-
  health concern flags, supply procurement) based on field state and
      ...
      (case op
        ...
        {:op :log-field-record
         ...
         :value {:field-id field-id
                 :acreage (:acreage request 0)
                 :fibre-yield (:fibre-yield request)
                 :quality-grade (:quality-grade request "ungraded")
                 :crop (:crop request "unspecified")
                 :record-type (:record-type request "planting")}
         :cites ["operator-submitted-field-data"]
         :summary "Field planting/fibre-yield/quality-grade record entry logged from operator submission"
         :confidence 0.9}
        ...
        {:op :schedule-field-operation
         ...
         :summary "Field operation (planting/defoliation/retting/harvest) proposed per operator request"
         :confidence 0.85}
        ...
        {:op :flag-crop-health-concern
         ...
         :summary "Crop pest (e.g. boll weevil)/disease/drought-stress concern flagged for agronomist/farmer review"
         :confidence 0.8}
